...
I am selecting to represent my rectangle with the bottom left and top right points. The edges of the rectangle will be forced to be
parallel with the axes as in the bottom and top of the rectangle is parallel to the x-axis and the left and right sides
are parallel to the y-axis. The coordinates for the bottom left and top right corners will be x and y coordinates represented as doubles.
A rectangle is only valid if the top right corner is up and to the right of the bottom left corner. This would make any lines
or configurations where the top right point is below the bottom left(or others of this sort) are invalid, as well as singular points.
The rectangle is allowed to be invalid at points in time, however most of the methods are not expected to work on an invalid rectangle. Having
it have the ability to be invalid makes it easier for the user to change the coordinates in whatever order they choose(don't have to change the 
bottom left before the top right if you are translating the rectangle down).
